...
def AddFileZip(nameZip, nameFile, firstTxt, secondTxt):
    z = zipfile.ZipFile(nameZip, 'w')
    z.writestr(nameFile, firstTxt)
    z.writestr('requirements.txt', secondTxt)
    z.close()
...
def CreateFirstFunc():
    ...
    result = str(subprocess.Popen(prog_config, stdin=subprocess.PIPE, stdout=subprocess.PIPE).communicate())
    print(Fore.GREEN + re.findall('http_invoke_url: \w*.\w*.\w*.\w*.\w*.\w*.\w*',result)[0].replace('http_invoke_url: ', ''))
...
def handler(event, context):
    ...
    requirements = """boto3==1.13.10
botocore==1.16.10
python-dateutil==2.8.1
pytz==2020.1"""

    AddFileZip('my-first-function.zip','index.py',text,requirements)
    # file = open('index.py', "w+")
    # file.write(text)
    # file.close()
    print('Файл index.py успешно загружен в архив my-first-function.zip')
...
pytz==2020.1"""
    ...
    # file.write(text)
    # file.close()
    AddFileZip('parrot.zip','parrot.py',text,requirements)
    print('Файл parrot.py успешно добавлен в архив parrot.zip')
...
def CreatePostgre():
    ...
    if connectIdAndDbHostId == "":
        connectIdAndDbHostId = input("Введите идентификатор подключения ")
...
CONNECTION_ID = "{connectIdAndDbHostId}"
DB_USER = "user1"
DB_HOST = "{connectIdAndDbHostId}.postgresql-proxy.serverless.yandexcloud.net"
...
def entry(event, context):
    ...
    prog_config = ['powershell',
                   ...
                   f' --environment CONNECTION_ID={connectIdAndDbHostId}'
                   ' --environment DB_USER=user1'
                   f' --environment DB_HOST={connectIdAndDbHostId}.postgresql-proxy.serverless.yandexcloud.net'
    ...
    subprocess.Popen(prog_config, stdin=subprocess.PIPE, stdout=subprocess.PIPE).wait()
    input("Ввод для удаления триггера")
...
paths:
  /hello:
    ...
    result = str(subprocess.Popen(prog_config, stdin=subprocess.PIPE, stdout=subprocess.PIPE).communicate())
    finalUrl = 'https://'+ re.findall('domain: \w*', result)[0].replace('domain: ', '')+ '.apigw.yandexcloud.net/results?secret=cecfb23c-bc86-4ca2-b611-e79bc77e5c31'
...
paths:
  /results:
    ...
    input(finalUrl + "\nКопируй ссылку и заходи на неё")
    menu()
...
simplejson==3.17.6"""
    ...
    # file.write(text)
    # file.close()
    AddFileZip('my-url-receiver-function.zip','my-url-receiver-function.py',text,requirements)
    print('Файл my-url-receiver-function.py успешно добавлен в архив my-url-receiver-function.zip')
...
def handler(event, context):
    ...
    AddFileZip('function-for-url-from-mq.zip', 'function-for-url-from-mq.py', text, requirements)
    print('Файл успешно перезаписан')
    ...
    prog_config = ['powershell',
                   ...
                   '--environment DB_USER=user1 '
                   f'--environment DB_HOST={connectIdAndDbHostId}.postgresql-proxy.serverless.yandexcloud.net '
                   f'--environment AWS_ACCESS_KEY_ID={keyId} '
                   f'--environment AWS_SECRET_ACCESS_KEY={secret} '
                   f'--environment QUEUE_URL={queueUrl} '
...
def CreateFinal():
    ...
    result = str(subprocess.Popen(prog_config, stdin=subprocess.PIPE, stdout=subprocess.PIPE).communicate())
    print('Техническая пауза в 5 секунды ')
    time.sleep(5)
...
def handler(event, context):
    ...
    requirements = '''ydb==0.0.41
protobuf==3.20.0'''

    AddFileZip('src.zip','index.py',text,requirements)
...
paths:
  ...
  /r/{{id}}:
    ...
    result = str(subprocess.Popen(prog_config, stdin=subprocess.PIPE, stdout=subprocess.PIPE).communicate())
    print('Копируй домен:\n' + Fore.GREEN + re.findall('domain: \w*.\w*.\w*.\w*',result)[0].replace('domain: ', ''))
...
def CreateServiceAccounts():
    ...
    idAccount1 = GettingIDServiceAccount('service-account-for-cf')
    idAccount2 = GettingIDServiceAccount('serverless-shortener')
    idAccount3 = GettingIDServiceAccount('ffmpeg-account-for-cf')

    AddRoleAccount(idFolder, idAccount1, 'editor storage.editor serverless.mdbProxies.user')
    AddRoleAccount(idFolder, idAccount2, 'storage.viewer ydb.admin editor')
    AddRoleAccount(idFolder, idAccount3, 'storage.viewer storage.uploader ymq.reader ymq.writer ydb.admin '
                                         'serverless.functions.invoker lockbox.payloadViewer editor')
...
def menu():
    ...
    [1] Создание всех ролей и сервисных аккаунтов
    [2] Создание первой функции. 2 Практическая
    [3] Создание index.py. 3 Практическая
    [4] Создание parrot.py. 4 Практическая
    [5] Создание кластера Postgre. 5 Практическая
    [6] Создание Gateway. 6 Практическая
    [8] Создание скриптов. 8 Практическая
    [9] Создание скриптов для проверки доступности. 9 Практическая
    [10] Создание файлов для Lockbox. 10 Практическая
    [11] Создание файлов для Финальной задачи. 11 Практическая
    [12] Просмотр данных активного аккаунта
    Введите число: """)
